                ...
                <div class="flex justify-between items-center mb-4">
                    <div class="flex items-center space-x-4">
                        ...
                        <label for="links" class="cursor-pointer">
                            <input type="file" name="links[]" id="links" multiple class="hidden">
                            <svg  width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script>
    ...
    function previewDocuments() {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (file.type.includes('application/pdf') || file.type.includes('text/plain') || file.type.includes('application/msword') || file.type.includes('application/vnd.openxmlformats-officedocument.wordprocessingml.document')) {
                ...
                var documentIcon = document.createElement('svg');
                documentIcon.setAttribute('fill', '#000000');
                documentIcon.setAttribute('version', '1.1');
                documentIcon.setAttribute('id', 'Capa_1');
                documentIcon.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
                documentIcon.setAttribute('xmlns:xlink', 'http://www.w3.org/1999/xlink');
                documentIcon.setAttribute('width', '24px');
                documentIcon.setAttribute('height', '24px');
                documentIcon.setAttribute('viewBox', '0 0 31.612 31.612');
                documentIcon.setAttribute('xml:space', 'preserve');

                // Vous pouvez ajouter ici le code SVG de l'icône du document, ou simplement un texte indiquant le type de document
                ...
                closeButton.classList.add('absolute', 'top-0', 'right-0', 'mt-1', 'mr-1', 'text-white', 'bg-gray-500', 'rounded-full', 'hover:bg-red-600');
                closeButton.onclick = function() {
                    documentPreview.remove(); // Supprimer l'aperçu du document lorsqu'on clique sur le bouton de fermeture
                };
    ...
    // Ajout d'un gestionnaire d'événement pour déclencher la fonction de prévisualisation lorsqu'un document est sélectionné
    ...
    document.getElementById('links').addEventListener('click', function() {
        ...
        if (link !== null && link.trim() !== "") {
            ...
            if (urlPattern.test(link)) {
                // Mettre à jour la valeur de l'élément input pour les liens
                document.getElementById('links').value = link;
                // Afficher le lien dans une zone de texte à des fins de démonstration
                document.getElementById('linkDisplay').textContent = "Lien saisi : " + link;
            } else {
